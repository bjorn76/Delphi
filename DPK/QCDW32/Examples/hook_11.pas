...
procedure SaveAxesHandles(x,y: THANDLE);
...
procedure ChangeAxesScale (pPageDesc: PPAGE_DEF;wParam: UINT);
VAR
    nAxisXScale, nAxisYScale: INTEGER;
    pGrDesc: PGRAPH_DEF;
BEGIN
    pGrDesc := WGGetSelectedGraph (pPageDesc);
    nAxisXScale := WGGetAxisScale (pGrDesc, hAxX);
    nAxisYScale := WGGetAxisScale (pGrDesc, hAxY);

	case (wParam) of
	IDM_XLIN_YLIN:  BEGIN
        	{  changes the x axis to linear, y axis to linear }
			if (nAxisXScale = ST_LOG) THEN
				WGChangeAxisScale (pGrDesc, hAxX, ST_LINEAR, TRUE);
			if (nAxisYScale = ST_LOG) THEN
				WGChangeAxisScale (pGrDesc, hAxY, ST_LINEAR, TRUE);
			END;

     IDM_XLIN_YLOG: BEGIN
     		{ changes the x axis to linear, y axis to log (semilog) }
			if (nAxisXScale = ST_LOG) THEN
				WGChangeAxisScale (pGrDesc, hAxX, ST_LINEAR, TRUE);
			if (nAxisYScale = ST_LINEAR) THEN
				WGChangeAxisScale (pGrDesc, hAxY, ST_LOG, TRUE);
	    	 end;

     IDM_XLOG_YLOG: BEGIN
                { changes the x axis to log, y axis to log }
         		if (nAxisXScale = ST_LINEAR) THEN
				WGChangeAxisScale (pGrDesc, hAxX, ST_LOG, TRUE);
			if (nAxisYScale = ST_LINEAR) THEN
				WGChangeAxisScale (pGrDesc, hAxY, ST_LOG, TRUE);
            end;
	END;
...
{ *********************
 here's where the added menu items are processed.
 Refer to axscale.pas for the code to the function ChangeAxesScale. }
	IDM_XLIN_YLIN,
    IDM_XLIN_YLOG,
	IDM_XLOG_YLOG:	 ChangeAxesScale (pPageDesc, wParam);
